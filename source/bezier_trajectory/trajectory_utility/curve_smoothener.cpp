#include "trajectory_utility/curve_smoothener.h"

#include <iostream>
#include "trajectory_utility/smooth_motion_profile.h"

CurveSmoothener::CurveSmoothener(std::function<GripperPose(double)> curve, double endTime) :
  originalCurve_(curve), endTime_(endTime)
{
  ...
  equalizedCurve_ = [this](double t) -> GripperPose
  {
  ...
  smoothCurve_ = [this](double t) -> GripperPose
  {
...
std::function<GripperPose(double)> CurveSmoothener::equalizedCurve()
{
  return equalizedCurve_;
}

std::function<GripperPose(double t)> CurveSmoothener::smoothCurve()
{ 
  return smoothCurve_;
}
